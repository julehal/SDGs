...
# Kanten mit Gewichten basierend auf der Anzahl der Interaktionen hinzufügen, aber nur für Synergy, Trade-off und n.a.
for _, row in df.iterrows():
    if row['Type'] in ['synergy', 'trade-off']:
        if G.has_edge(row['GoalA'], row['GoalB']):
            G[row['GoalA']][row['GoalB']]['weight'] += 1
        else:
...
for u, v, data in G.edges(data=True):
    if data['type'] == 'synergy':
        edge_colors.append('#b7d5ac')
    elif data['type'] == 'trade-off':
        edge_colors.append('#ff7d82')
...
plt.title('Netzwerkanalyse der SDG-Interaktionen (Circular Layout, farbige Knoten und Kanten für Synergien, Trade-offs und n.a.)')
plt.show()
